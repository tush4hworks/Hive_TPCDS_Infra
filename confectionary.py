...
class confectionary:
	...
	def changeConfig(self,s_option):
		try:
			...
			print 'Exception while adding configurations {}. Please check input\n'.format(e.__str__())
	...
	def interact(self):
		...
		while True:
			...
			if option=='P':
				...
				self.deleteConfig(raw_input('Enter config name to be deleted\n'))
				print 'Done.'
			elif option=='M':
				...
				self.currentConfigs()
				if raw_input ('Are you sure to lock above configurations? Press (Y) or (N)\n')=='Y':
					break
			else:
		...
		print 'Starting Tests with chosen configurations'
		self.currentConfigs()
